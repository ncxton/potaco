...
The agent looks for these ten categories. The first three are stylistic, the next three are structural, the next two are about hidden cost, the ninth is about behavior coverage, and the tenth is structural.
...
### Structural
10. **Oversized modules** — any source file exceeding **250 pure LOC** (non-blank, non-comment lines). This is an architectural defect, not a style preference. Measure: `awk '!/^[[:space:]]*$/ && !/^[[:space:]]*(#|\/\/)/' | wc -l`.

   **When found, do NOT just flag it. Execute a full modular refactoring:**
   1. Run the LOC checker recursively on scope to list all violations.
   ...
   6. Verify: run the LOC checker again — every file must be ≤250 pure LOC. Run tests, typecheck, lint.
...
### Phase 4: Parallel slop removal via agents in batches of 5

Files are processed by agents with the `remove-ai-slops` skill loaded, **batched 5 at a time in parallel**. Use the `Task` tool to dispatch parallel worker droids, each loaded with this skill. The 5-wide batch is the sweet spot — more than 5 creates result-merging noise and context contention, fewer wastes parallelism.

**Batching protocol** (strict):

1. Slice the in-scope file list into chunks of up to 5 files.
2. For each chunk, launch all `Task` calls **in a single message**, every one with `fireAndForget=false` (wait for results).
3. Once all 5 in the batch complete, collect each result.
...
- Duplication: copy-pasted branches, redundant helpers
...
**Batch failure handling**: a timeout only means the agent is still working, not that it failed. For long passes, require each child to report progress. Treat a running child as alive. Mark a file for retry only when the child completes without the deliverable or reports being blocked. Do NOT block the remaining 4 in that batch; collect successful results and retry the failed file once later. If retry also fails, escalate that file under "Issues Found & Fixed" in the final report.

### Phase 5: Verify with quality gates + critical review

Run the five quality gates listed above using the `Execute` tool. Then walk the critical review checklist:
...
- If correctness depends on further inspection, keep using the test runner, `Grep`, `Glob`, `Read`, and direct file reads until the result is grounded.

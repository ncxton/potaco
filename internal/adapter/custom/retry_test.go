...
func TestRetryOn429ThenSuccess(t *testing.T) {
	...
	ca := a.(*Adapter)
	ca.SetBackoff(func(int) time.Duration { return 1 * time.Millisecond })
...
func TestRetryAfterHeaderHonored(t *testing.T) {
	...
	ca.SetBackoff(func(int) time.Duration { return 100 * time.Hour }) // should NOT be used
	ca.SetSleep(func(ctx context.Context, d time.Duration) {
		sleptDuration = d
	})
...
func TestContextCancellationDuringBackoff(t *testing.T) {
	...
	ca := a.(*Adapter)
	ca.SetBackoff(func(int) time.Duration { return 50 * time.Millisecond })

	ctx, cancel := context.WithCancel(context.Background())
	ca.SetSleep(func(c context.Context, d time.Duration) {
